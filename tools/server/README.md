...
Multi-token requests use one task and one slot. All first-token scores are stored after the initial prompt prefill. The first multi-token candidate uses that fresh state; before each later multi-token candidate, the slot's sequence is cleared and the exact original prompt tokens are prefilled again. Single-token candidates use their stored scores without extra decoding. An M-token choice needs M-1 forced token evaluations, one per scheduler step; the final token is scored without decoding it. No sampler or speculative decoding is used. Generated-token counters remain zero. `n_decode_total` includes prompt and forced-prefix decode calls; `prompt_tokens_total` counts every prompt prefill.

Limits are 256 choices, 1 MiB of total choice text (UTF-8 bytes), and 32768 total choice tokens per request. The prompt must pass the existing prompt context check, and `prompt_token_count + max_choice_token_count - 1` must fit the assigned slot's context. Decision requests are never shifted or truncated, even if context shifting is enabled. Invalid input and context overflow return HTTP 400; evaluation failure, non-finite target logits, or an invalid normalization row return HTTP 500. Other vocabulary entries may have negative-infinity logits (zero mass). Embedding-only servers are unsupported. Cancellation or evaluation failure clears multi-token decision state before the slot is reused.

The v0.3.1 correctness patch replaces decision snapshot reuse with prompt recomputation. Later GPT-OSS CUDA/SWA tests found candidate-order-dependent scores after snapshot restoration, despite matching logical attention inputs. Re-prefill recovered oracle agreement in the diagnostic tests, at increased prompt-processing cost. The single-token fast path and response formats are unchanged; the historical v0.3 validation below is not validation of the patched binary.
...
The independent `test-save-load-state --decision-oracle` tool creates a fresh context for each candidate. It does not use the server's candidate-state reuse or scoring helpers. The oracle was corrected to decode the complete prompt separately, score the first candidate token from the final prompt output, and then decode the forced candidate prefix. This prevents `n_batch` from accidentally merging prompt and candidate tokens into one decode call. In the Qwen reference case, merging these phases had changed the second `New York` token score by approximately 0.019 nats; the separate phases produced the matching scores above.
